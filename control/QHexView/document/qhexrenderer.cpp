...
int QHexRenderer::headerLineCount() const { return 0; }
...
QString QHexRenderer::hexString(quint64 line, QByteArray* rawline) const
{
    ...
    QString result;
    for( int i = 0; i < lrawline.size(); i++ )
    {
        result.append( lrawline.mid( i, 1 ).toHex() );
        result.append( " " );
    }
    return result;
...
int QHexRenderer::getAddressWidth() const
{
    ...
    else if(maxAddr <= 0xFFFFFFFF) return 8;
    else return 16;
}
